    ...
    <div class="row">
        ...
        <form action="{{route('result')}}" method="post" enctype="multipart/form-data">
            ...
            <h5>{{$key +1 }} ) {{$mq->question->question}}</h5>
                    ...
                    <input type="checkbox" {{is_correct_mcq($result->id,$mq->question->id,$answer->id)?'checked':''}} disabled name="mcq[]" value="{{$mq->question_id}}-{{$answer->id}}"  class="form-check-input" id="exampleCheck1">
                    <label class="form-check-label {{$answer->is_correct == 1 ? 'text-success':''}}" for="exampleCheck1">{{$answer->answer}}</label>
                      </div>
                    @empty 
